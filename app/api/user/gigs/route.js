...
export async function GET(req) {
  ...
  console.log("[Backend] Query params:", { status, page, limit, search, sort })
  ...
  try {
    ...
    switch (sort) {
      case "recent":
        ...
        break
      case "performance":
        sortOptions = { orders: -1, clicks: -1 }
        break
      case "price":
        sortOptions = { "packages.0.price": -1 }
        break
      case "orders":
    ...
    // Execute queries
    const [gigs, total] = await Promise.all([
      Gigs.find(query).sort(sortOptions).skip(skip).limit(limit).lean(),
      Gigs.countDocuments(query),
    ])
